...
export type CheckoutStep = 'customerInfo' | 'shippingDelivery' | 'reviewAgreement' | 'payment' | 'confirmation';
...
const stepOrder: CheckoutStep[] = ['customerInfo', 'shippingDelivery', 'reviewAgreement', 'payment', 'confirmation'];
...
const useCheckoutStepsStore = create<CheckoutStore>()(
    devtools((set, get) => ({
        ...
        nextStep: () => {
            ...
            if (currentIndex < stepOrder.length - 1 && steps[currentStep].canProceed) {
